...
def generate_one_completion(iterpath, language, problem, index, code_data_list, pca, k):
    task = problem['prompt']
    declaration = problem['declaration']

    id_index = problem['task_id']
    id_index = id_index[:3] + '_' + id_index[4:] 
    ...
    code = extract_function_body(code_message, language)
    return code
        ...
        try:
            ...
            for task_id in tqdm(problems):
                if int(task_id[shift:]) < int(start_task_id):
                    ...
                    completion_with_code = generate_one_completion(iterpath, language, problems[task_id], index, code_data_list, pca, k)
