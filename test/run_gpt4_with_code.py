...
A relevant code might be:\n
{}
You can refer to the above knowledge to do the completion. Here is the problem:\n
\n
{}
'''.strip().format(language, knowledge, question.strip())

def generate_one_completion(language, problem, index, code_data_list, pca, k):
    task = problem['prompt']
    ...
    message = code_response.choices[0]["message"]["content"]
        ...
        try:
            ...
            for task_id in tqdm(problems):
                if int(task_id[shift:]) < int(start_task_id):
                    ...
                    completion_with_code = generate_one_completion(language, problems[task_id], index, code_data_list, pca, k)
            ...
            write_jsonl(os.path.join(output_path, 'samples_with_code.jsonl'), samples)
