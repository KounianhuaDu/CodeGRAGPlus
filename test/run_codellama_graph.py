...
def generate_one_completion(language, problem, index, graph_data_list, pca, k, generator):
    task = problem['prompt']
    ...
    response = generator.chat_completion(
        prompt_graph,  # type: ignore
        max_gen_len=1024,
        temperature=0.0,  # 调整生成文本的随机性
        top_p=0.0
    )
    graph_message = response["generation"]["content"]
        ...
        try:
            ...
            for task_id in tqdm(problems):
                if int(task_id[shift:]) < int(start_task_id):
                    ...
                    completion_with_graph = generate_one_completion(
                        language, problems[task_id], index, graph_data_list, pca, k)
                    graph_temp_dict = dict(task_id=task_id, generation=completion_with_graph,
            ...
            write_jsonl(os.path.join(
                output_path, 'samples_with_graph.jsonl'), samples)
